...
- engine(): lazy SQLAlchemy engine using PG_URI
- fetch_targets(): load active targets for a client & period (backward compatible)
- fetch_client_bq_config(): read BQ project/dataset/table_prefix from clients.dataslayer_config
- (optional) onboarding helpers: client_exists, upsert_client_config, upsert_target
"""
...
def _targets_table_name() -> str:
    """
    Returns the targets table name (defaults to 'targets') and validates it.
    """
...
# (backward compatible if bounds/currency not present)
# --------------------------------------------
def fetch_targets(
    client_id: str,
    metrics: List[str],
...
def upsert_target(
    ...
    """
    Idempotent insert of a target row. Call this 3x for roas/revenue/cost during onboarding.
    """
    targets_tbl = _targets_table_name()
    sql = text(f"""
